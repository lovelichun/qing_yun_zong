<template>
  <div class="spirit-material-container">
    <el-card>
      <template #header>
        <div class="card-header">
          <span>灵材管理</span>
          <el-button type="primary" @click="handleAdd">新增灵材</el-button>
        </div>
      </template>
...
const handleAdd = () => { dialogTitle.value = '新增灵材'; Object.assign(form, { id: null, category: CATEGORY, level: '', grade: '', name: '', quantity: 0, description: '' }); dialogVisible.value = true }
const handleEdit = (row) => { dialogTitle.value = '编辑灵材'; Object.assign(form, row); dialogVisible.value = true }
...
const handleDelete = async (id) => {
  try {
    await ElMessageBox.confirm('确定要删除该灵材吗?', '提示', { type: 'warning' })
...
<style scoped>
.spirit-material-container { padding: 20px; }
