    ...
    <el-card>
      <template #header>
        <div class="card-header">
          <span>订单管理</span>
          <el-button v-hasPermi="'order:add'" type="primary" @click="handleAdd">新增订单</el-button>
        </div>
      </template>
      ...
      <el-table :data="tableData" border style="width: 100%">
        ...
        <el-table-column label="操作" width="180" fixed="right">
          <template #default="{ row }">
            <el-button v-hasPermi="'order:update'" link type="primary" @click="handleEdit(row)">编辑</el-button>
            <el-button v-hasPermi="'order:delete'" link type="danger" @click="handleDelete(row.id)">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
...
const queryForm = reactive({ category: '', name: '' })
const pagination = reactive({ pageNum: 1, pageSize: 10, total: 0 })
const tableData = ref([])
const dict = ref({ categories: [], levels: [], grades: [] })
const dialogVisible = ref(false)
const dialogTitle = ref('新增订单')
const formRef = ref(null)
const form = reactive({ id: null, category: '', level: '', grade: '', name: '', quantity: 0, description: '' })
const rules = {
...
const loadData = async () => {
  try {
    const res = await getOrderList({ pageNum: pagination.pageNum, pageSize: pagination.pageSize, ...queryForm })
...
const handleReset = () => { Object.assign(queryForm, { category: '', name: '' }); handleQuery() }
const handleSizeChange = () => loadData()
const handleCurrentChange = () => loadData()
const handleAdd = () => { dialogTitle.value = '新增订单'; Object.assign(form, { id: null, category: '', level: '', grade: '', name: '', quantity: 0, description: '' }); dialogVisible.value = true }
const handleEdit = (row) => { dialogTitle.value = '编辑订单'; Object.assign(form, row); dialogVisible.value = true }

const handleSubmit = async () => {
  await formRef.value.validate(async (valid) => {
    if (valid) {
      try {
        if (form.id) { await updateOrder(form); ElMessage.success('更新成功') }
        else { await addOrder(form); ElMessage.success('新增成功') }
...
const handleDelete = async (id) => {
  try {
    ...
    await deleteOrder(id); ElMessage.success('删除成功'); loadData()
