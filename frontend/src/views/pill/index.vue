    ...
    <el-card>
      <template #header>
        <div class="card-header">
          <span>丹药管理</span>
          <el-button type="primary" @click="handleAdd">新增丹药</el-button>
        </div>
      </template>
...
const handleAdd = () => { dialogTitle.value = '新增丹药'; Object.assign(form, { id: null, category: CATEGORY, level: '', grade: '', name: '', quantity: 0, description: '' }); dialogVisible.value = true }
const handleEdit = (row) => { dialogTitle.value = '编辑丹药'; Object.assign(form, row); dialogVisible.value = true }
...
const handleDelete = async (id) => {
  try {
    await ElMessageBox.confirm('确定要删除该丹药吗?', '提示', { type: 'warning' })
